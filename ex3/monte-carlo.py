...
class MonteCarloSimulator:
  ...
  def runSingle(self, count=-1, verbose=False):
    if count == -1: count = self.unitTestSize
    for i in range(count):
      ...
      if MonteCarloSimulator.isInsideCircle(rx, ry):
        self.countCircle += 1
        if verbose: print("count={}".format(i), rx, ry)
        if self.file: self.file.write("%f,%f\n" % (rx, ry))
      if self.file: self.file.close()

    estimatedPi = 4 * self.countCircle / self.countTotal
    print('total={}, in-circle={}, estimated Pi={}'
      .format(self.countTotal, self.countCircle, estimatedPi))
