...
m1 = tf.Variable(tf.random_uniform(
    [3, dimention], minval=0.1, maxval=0.9, dtype=tf.float32))
b1 = tf.Variable(tf.random_uniform(
    [dimention], minval=0.1, maxval=0.9, dtype=tf.float32))
h1 = tf.sigmoid(tf.matmul(x0, m1) + b1)

# Layer 2: dimension x 1 sigmoid output
m2 = tf.Variable(tf.random_uniform(
    [dimention, 1], minval=0.1, maxval=0.9, dtype=tf.float32))
b2 = tf.Variable(tf.random_uniform(
    [1], minval=0.1, maxval=0.9, dtype=tf.float32))
